...
def BlastChecker(variable):
    ...
    for line in variable.readlines():
        ...
        # Else, build dictionary of variables in tab.
        else:
...
def alignment_to_subject_length_k(dict, k = 0):
    # Check whether alignment length within minimum k threshold to subject length.
...
def mismatch_k(dict, k = 0):
    # Takes into account possibility of k mismatch(es) as okay for classification.
...
def hit_start_end_k(dict, k = 0):
    # Reduces need for alignment length being the same as subject length by k value.
